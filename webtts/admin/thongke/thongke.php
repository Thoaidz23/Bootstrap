    ...
    <div class="small-box" style="background-color: #6f4f28;">
        <div class="inner">
            <h3 style="color: white">
                <?php
                    $sql_low_stock = "SELECT COUNT(*) AS low_stock FROM tbl_sanpham WHERE soluong < 50";
            ...
            <div class="table-responsive">
                <table class="table m-0">
                    ...
                    <?php
                        ...
                        $sql = "
                            SELECT od.id_sanpham, SUM(od.soluongmua) AS total_sold 
                        ...
                        if ($query && mysqli_num_rows($query) > 0) {
                            while ($row = mysqli_fetch_assoc($query)) {
                                ...
                                $product_query = mysqli_query($mysqli, "SELECT ten_sanpham, giasp FROM tbl_sanpham WHERE id_sanpham = '$id_sanpham'");
                                if ($product_query && mysqli_num_rows($product_query) > 0) {
                                    $product_row = mysqli_fetch_assoc($product_query);
                                    $product_name = $product_row['ten_sanpham'];
                                    $product_price = $product_row['giasp'];

                                    $total_revenue = number_format($total_sold * $product_price, 0, ',', '.') . ' đ'; // Định dạng tiền tệ
            ...
            <div class="table-responsive">
                <table class="table m-0">
                    ...
                    <?php
                        ...
                        $sql = "
                            SELECT dm.id_dmsp, dm.ten_dmsp, 
                                   COUNT(sp.id_sanpham) AS total_products, 
                                   SUM(od.soluongmua) AS total_sold, 
                                   SUM(od.soluongmua * sp.giasp) AS total_revenue
                            FROM tbl_danhmucsanpham dm
                            JOIN tbl_sanpham sp ON dm.id_dmsp = sp.id_dmsp
                            JOIN tbl_order_detail od ON sp.id_sanpham = od.id_sanpham
                            JOIN tbl_order o ON od.code_order = o.code_order
                            WHERE o.status = 3
                            GROUP BY dm.id_dmsp
                            ORDER BY total_sold DESC
