<?php
    ...
    $sql_base = "FROM tbl_sanpham, tbl_danhmucsanpham WHERE tbl_danhmucsanpham.id_dmsp = tbl_sanpham.id_dmsp";

    $sql_saphethang = "SELECT COUNT(*) FROM tbl_sanpham WHERE soluong < 50";
    ...
    if ($filter == 'saphethang') {
        $sql_base .= " AND tbl_sanpham.soluong < 50";
    }

    if (!empty($keyword)) {
        $sql_base .= " AND (tbl_sanpham.ten_sanpham LIKE '%$keyword%' OR tbl_sanpham.masp LIKE '%$keyword%')";
    }
    ...
    $sql_lietke_sp = "SELECT * " . $sql_base . " ORDER BY tbl_sanpham.id_sanpham DESC LIMIT $start, $limit";
...
<div class="col-lg-12">
    <div class="card">
        <div class="card-body d-flex justify-content-between align-items-center">
            ...
            <a href="http://localhost/tts/admin/admin.php?action=quanlysanpham&query=them" class="btn btn-primary">
            ...
            <table class="table table-striped table-hover" style="width: 100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><img src="./quanlysanpham/uploads/<?php echo $row_sp["hinhanh"]; ?>" width="150px"></td>
                        <td><?php echo number_format($row_sp["giasp"], 0, ",", "."); ?><sup>đ</sup></td>
                        <td><?php echo $row_sp["soluong"]; ?></td>
                        <td><?php echo $row_sp["ten_dmsp"]; ?></td>
                        <td><?php echo $row_sp["masp"]; ?></td>
                        <td>
                            <?php echo ($row_sp["tinhtrang"] == 1) ? "Kích hoạt" : "Ẩn"; ?>
                        </td>
                        <td>
                            <div class="btn-group" role="group">
                                <a href="?action=quanlysanpham&query=xem&id_sanpham=<?php echo $row_sp['id_sanpham']; ?>" class="btn btn-primary me-2">Xem</a>
                                <a href="?action=quanlysanpham&query=sua&id_sanpham=<?php echo $row_sp['id_sanpham']; ?>" class="btn btn-warning me-2">Sửa</a>
                                <a href="quanlysanpham/xuly.php?query=xoa&id_sanpham=<?php echo $row_sp['id_sanpham']; ?>" class="btn btn-danger me-2">Xóa</a>
                            </div>
                        </td>
                    </tr>
                    <?php } ?>
